...
%         saveas(f, strcat(pathOut, sprintf('res%d.jpg',i)));
        ...
        csvwrite(strcat(pathOut, sprintf('%d.csv', i)), [data(idxNormals, 1:end-1), data(idxNormals, end)]);
        csvwrite(strcat(pathOut, sprintf('o%d.csv', i)), [data(idxOutliers, 1:end-1), data(idxOutliers,end)]);        
    end   
end