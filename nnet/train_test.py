...
def train_NN(train, labels, outlier_frac = 0, outlier_method = 'OCSVM', use_calibration = False,
		use_rescale_priors = False, **parameters):
	if outlier_frac:
...
def train_test_NN(train, labels, test, use_rescale_priors = False, normalize_log = True, **parameters):
	train, test = conormalize_data(train, test, use_log = normalize_log)
	net = train_NN(train, labels, normalize_log = normalize_log, use_rescale_priors = use_rescale_priors, **parameters)
...
def make_pretrain(pretrain_path, data, labels, minimum_train_loss = 0.7, normalize_log = True, **params):
	if not isfile(pretrain_path):
		...
		train, test = conormalize_data(data, labels, use_log = normalize_log)
